...
%             obj = obj.get_refscale;   % get the scale matrix for reference traj.
        ...
        function results = run_trial_mpc( obj , ref , x0 , u0 , load_value )
            ...
            if obj.mpc.loaded
                if nargin < 5
                    ...
                    if size( load_value , 2 ) ~= obj.sys.params.nw
                        error(['Load argument should have ' , obj.sys.params.nw , ' columns, not ' , size( load_value , 2) ]);
                    end
                    if isempty(load_value)
                        w = zeros( size(ref,1) , obj.sys.params.nw );
                    elseif size( load_value , 1 ) == 1    % assume constant load
                        w = kron( ones( size(ref,1) , 1 ) , load_value );
                    elseif size( load_value , 1 ) ~= size( ref , 1 )
                        error('Load argument must have 1 or the same number of rows as ref argument');
                    else
                        w = load_value;
                    end
                end
            elseif exist( 'load_value' , 'var' )   % model doesn't have load, but I want to apply one to actual system
                if size( load_value , 1 ) == 1    % assume constant load
                    w = kron( ones( size(ref,1) , 1 ) , load_value );
                elseif size( load_value , 1 ) ~= size( ref , 1 )
                    error('Load argument must have 1 or the same number of rows as ref argument');
                else
...
%                 y_kp1 = obj.scaleup.y( y_kp1_sc' )';  % scale output back up 
                ...
                if obj.mpc.loaded || exist( 'load_value' , 'var')
